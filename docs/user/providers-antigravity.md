...
must sit next to that executable, at the same version. T3 launches the executable
directly so ACP stdio does not go through `cmd.exe`. Leave the field blank to use
